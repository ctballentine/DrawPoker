...
include DrawPokerLogic
  ...
  def edit
    ...
    if @game.winner == nil
      ...
      # FALLBACK SAFEGUARD: Prevents edit.js.erb from crashing if game is already won
      @game.final_player_hand ||= @game.initial_player_hand
      @game.final_computer_hand ||= @game.initial_computer_hand
      @game.final_player_hand_rank ||= @game.initial_player_hand_rank
      @game.final_computer_hand_rank ||= @game.initial_computer_hand_rank
    end

    render action: 'edit', content_type: 'text/javascript'
  end
end
